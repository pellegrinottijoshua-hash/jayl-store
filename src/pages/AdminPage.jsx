...
function AddProductTab({ editingProduct, onSaved, onCancel }) {
    ...
    try {
      ...
      const res = await fetch('/api/publish-social', {
        ...
        body: JSON.stringify({
          ...
          imageUrl,
          link: `https://jayl.store/product/${productId || ''}`,
  ...
  return (
    <div className="space-y-5">
      ...
      {/* SEO & Social Keywords */}
      <Collapsible label="🔍 SEO & Social" defaultOpen={true}>
      <Card title="SEO & Social (keyword research)">
        ...
        <div className="space-y-4">
          ...
          <Field label="Pinterest caption" hint="Copy-paste to Pinterest pin description.">
            ...
            {/* Pinterest publish */}
            <div className="mt-3 space-y-2 border-l-2 border-red-900 pl-3">
              <p className="text-[10px] uppercase tracking-widest text-red-400 font-semibold">📌 Pubblica su Pinterest</p>
              <select
                value={pinterestBoardId}
                onChange={e => setPinterestBoardId(e.target.value)}
                onFocus={fetchPinterestBoards}
                className="w-full bg-gray-900 border border-gray-700 text-gray-300 text-xs px-3 py-2 focus:outline-none focus:border-red-700"
              >
                <option value="">— Seleziona board —</option>
                {loadingBoards && <option disabled>Caricando board…</option>}
              ...
              <button
                type="button"
                onClick={() => publishToPinterest(savedProductId, title)}
                disabled={pinterestPublishing || !pinterestBoardId}
                className="w-full flex items-center justify-center gap-2 bg-red-900 hover:bg-red-800 disabled:opacity-40 text-red-100 px-3 py-2 text-xs font-medium transition-colors"
              >
                {pinterestPublishing ? '📌 Pubblicando…' : '📌 Pubblica Pin'}
              </button>
              {pinterestMsg && <p className={`text-xs ${pinterestMsg.startsWith('✓') ? 'text-green-400' : 'text-red-400'}`}>{pinterestMsg}</p>}
              {pinterestPinUrl && <a href={pinterestPinUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-red-400 underline">→ Vedi pin su Pinterest</a>}
            </div>
          </Field>
        </div>
        ...
        <div className="border border-orange-900/50 bg-[#0d0a08] p-4 space-y-4">
            ...
            <div>
              ...
              <textarea value={etsyTitle} onChange={e => setEtsyTitle(e.target.value)} rows={2}
                placeholder="Generato dall'AI — clicca ✨ per compilare"
            ...
            <div>
              ...
              {etsyTags.length > 0 ? (
                ...
                <p className="text-gray-700 text-xs italic">Generato dall'AI — clicca ✨ per compilare</p>
              )}
            </div>
